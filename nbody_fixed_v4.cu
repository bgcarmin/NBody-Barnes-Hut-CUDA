...
// Paralelno ubacuje čestice u octree
                                      ...
                                      int *leafNodeIdx, const int *indices, int startIdx, int batchSize)
{
    ...
    while (depth < 25)
    {
        ...
        if (child == -1)
        {
            // Uspješno ubacena čestica kao leaf
            leafNodeIdx[idx] = nIdx;
            break;
        }
        else if (child < n)
        {
            // Postoji leaf, treba kreirati novi interni čvor
            int oldB = child;
            ...
            // Pokušaj ponovo ubaciti u novi interni čvor
            atomicExch(&node->children[oct], next);
            nIdx = next;
        }
        else
        {
            // Već postoji interni čvor, nastavi navigaciju
            nIdx = child;
        }
        depth++;
    }
...
__global__ void updateVisualsKernel(float *vbo_p, float *vbo_c, const float *posX, const float *posY, const float *posZ, const float *velX, const float *velY, const float *velZ, int n)
{
    ...
    if (i < n)
    {
        ...
        vbo_c[i * 3] = 0.3f + t * 0.7f;
        vbo_c[i * 3 + 1] = 0.2f + t * 0.5f;
        vbo_c[i * 3 + 2] = 1.0f - t * 0.5f;
    }
}
...
void simulationStep()
{
    ...
    // Paralelno ubaci čestice u batch-evima da se smanji contention
    int batchSize = 512;
    for (int start = 0; start < N; start += batchSize)
    {
        int currentBatch = min(batchSize, N - start);
        int batchBlocks = (currentBatch + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        insertParticlesKernel<<<batchBlocks, THREADS_PER_BLOCK>>>(d_posX, d_posY, d_posZ, d_nodes, N, d_nodeCounter, d_leafNodeIdx, d_indices, start, currentBatch);
        cudaDeviceSynchronize();
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (i < N * 0.15f)
        {
            ...
            h_m[i] = 20.0f + ((float)rand() / RAND_MAX) * 30.0f;
        }
        else
        {
        ...
        h_px[i] = r * cos(a);
        h_py[i] = r * sin(a);
        h_pz[i] = ((float)rand() / RAND_MAX - 0.5f) * (r * 0.12f);

        float cumulativeMass = 150000.0f * (r / 1000.0f);
        if (r < 150.0f)
            cumulativeMass = 80000.0f;
