...
{
    // Koristimo standardni grid-stride loop za sigurnost i skalabilnost
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if (i < nodeCount)
    {
        // Lokalno učitavanje u registre (brže od višestrukog pristupa globalnoj memoriji)
        float m = nodes[i].mass;

        // Koristimo epsilon za izbjegavanje dijeljenja s nulom,
        // ali i za preskakanje neaktivnih čvorova
        if (m > 1e-6f)
        {
            float invMass = 1.0f / m; // Dijeljenje je skupo, množenje je brzo
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (r < 150.0f)
            cumulativeMass = 80000.0f; // Jača sila u jezgri
