...
#define G_CONST 1.0f
#define THETA 0.5f // Smanjen za veću preciznost (manji THETA = sporije ali tačnije)
#define DT 0.01f
    ...
    while (ptr > 0)
    {
        ...
        else
        {
            for (int k = 0; k < 8; k++)
            {
                ...
                if (child >= n)
                    stack[ptr++] = child; // Unutrašnji čvor
                else if (child != -1)
                { // List (tijelo)
                    ...
                    float lf = G_CONST * 10.0f / (ld2 * ldist); // Pretpostavka mase 10
                    ax += lf * ldx;
                    ay += lf * ldy;
                    az += lf * ldz;
                }
...
int main()
{
    ...
    printf("Starting N-Body Benchmark (Barnes-Hut)\n");
    printf("N = %d bodies, %d iterations\n\n", N, iterations);
